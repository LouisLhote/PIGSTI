# Rscript sexing_residual_method_dog.R input.idx output.pdf output.tsv
...
{
	# Start with plot
	plot(d[c(1:39),i]~d$length[c(1:39)],xlab="Chromsome length (Mb)",ylab="Number of reads", main=colnames(d)[i],pch=16,col="slategrey",xaxt="n")
	points(d$length[39],d[39,i],col="red",pch=16)
	text(d$length[39],d[39,i],"X",col="red",pos=1)
	...
	text(d$length[30],d[30,i],"30",col="slategrey",pos=1, cex=0.5)
	text(d$length[31],d[31,i],"31",col="slategrey",pos=1, cex=0.5)
	text(d$length[32],d[32,i],"32",col="slategrey",pos=1, cex=0.5)
	text(d$length[33],d[33,i],"33",col="slategrey",pos=1, cex=0.5)
	text(d$length[34],d[34,i],"34",col="slategrey",pos=1, cex=0.5)
	text(d$length[35],d[35,i],"35",col="slategrey",pos=1, cex=0.5)
	text(d$length[36],d[36,i],"36",col="slategrey",pos=1, cex=0.5)
	text(d$length[37],d[37,i],"37",col="slategrey",pos=1, cex=0.5)
	text(d$length[38],d[38,i],"38",col="slategrey",pos=1, cex=0.5)
	points(d$length[39],(2*d[39,i]),col="lightgrey",pch=16)
	text(d$length[39],(2*d[39,i]),"2X",col="lightgrey",pos=3)
	axis(side=1,at=axTicks(1),labels=axTicks(1)/1000000)

	# Fit linear regression
	model<-lm(d[c(1:38),i]~d$length[c(1:38)])
	abline(model)

	# 95% CI lines for regression line
	prd <- predict( model, data.frame(d$length[c(1:38)]), interval="confidence", level=0.95)
	confidence <- data.frame( cbind( d$length[c(1:38)], prd[,2], prd[,3] ) )
	...
	modelwithX<-lm(d[c(1:39),i]~d$length[c(1:39)])
	studreswithX<-rstudent(modelwithX)
	femaleprob<-pt(studreswithX[39],df=36)
	modelwithtwiceX<-lm(c(d[c(1:38),i],(d[39,i]*2))~d$length[c(1:39)])
	studreswithtwiceX<-rstudent(modelwithtwiceX)
	maleprob<-pt(studreswithtwiceX[39],df=36,lower.tail=F)
